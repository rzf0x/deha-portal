...
class DashboardGuruUmum extends Component
{
    #[Title('Dashboard Guru Umum')]

    public $totalGuruUmum = 0;
    ...
    {
        $this->totalKategoriPelajaran = KategoriPelajaran::where('role_guru', 'umum')->count();
        $this->totalMataPelajaran = JadwalPelajaran::where('role_guru', 'umum')->count();
        ...
        $this->jadwalPiket = JadwalPiket::where('role_guru', 'umum')->with(['santri', 'kelas'])
            ->take(10)
            ->get();

        $this->jadwalPelajaran = JadwalPelajaran::where('role_guru', 'umum')->with(['kelas', 'kategoriPelajaran'])
            ->take(10)
            ->get();
    ...
    {
        return view('livewire.admin.e-santri.guru-umum.dashboard-guru-umum');
    }
}
